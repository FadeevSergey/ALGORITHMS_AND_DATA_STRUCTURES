...
int main(){
    ...
    sort(arrr.begin(), arrr.end(), [](pair<int, int> first, pair<int, int> second){return first.second < second.second;});
    sort(arrr.begin(), arrr.end());
//    for(int i = 0; i < n; i++)
//    {
//        cout << arrr[i].first << " " << arrr[i].second << "\n";
//    }

    for(int i = 0; i < n; i++)
    {
        arr.push_back(arrr[i].second);
    }
